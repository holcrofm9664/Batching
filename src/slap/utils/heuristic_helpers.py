from numpy import floor
from slap.comp_tests.dataclasses import (
    CageData
)

def quantity_that_fits(
    product:int, 
    demand:int, 
    weights_dict:dict[int,float], 
    vol_dict:dict[int,float], 
    cage_data:CageData, 
    c_weight:float, 
    c_vol:float
) -> int:
    """Checks the quantity of a product that can fit into an already part-filled cage.

    Args:
        product: Product whose units we are aiming to fit into the cage.
        demand: Total number we wish to fit in.
        weights_dict: Product weights.
        vol_dict: Product volumes.
        cage_data: Cage-related data.
        c_weight: Used weight capacity from already-assigned products.
        c_vol: Used volume capacity from already assigned products.
    
    Returns:
        Quantity of the product which will fit into the cage.
    """

    weight, volume = weights_dict[product], vol_dict[product]

    remaining_weight = cage_data.cage_weight_capacity-c_weight,
    remaining_volume = cage_data.cage_vol_capacity-c_vol

    # the quantity that can fit based on weight
    by_weight = demand if weight == 0 else floor(remaining_weight/weight)

    # the quantity that can fit based on volume
    by_volume = demand if volume == 0 else floor(remaining_volume/volume)

    quantity = int(max(0,min(demand, by_weight, by_volume)))

    return quantity


def add_product(
    product:int, 
    quantity:int, 
    shop_dem:int, 
    c_weight:float, 
    c_vol:float, 
    trips:dict[int,dict[int,list[int]]], 
    trip:int, 
    cage:int, 
    weights_dict:dict[int,float], 
    vol_dict:dict[int,float]
) -> tuple[float,float]:
    """Adds a pre-determined quantity of a product to a cage.

    Args:
        product: Product to be added.
        quantity: Number of units of that product which are to be added.
        shop_dem: Quantity of that product demanded by the shop.
        c_weight: Used weight capacity from already-assigned products.
        c_vol: Used volume capacity from already assigned products.
        trips: Current trips dictionary.
        trip: Trip number.
        cage: Cage number.
        weights_dict: Product weights.
        vol_dict: Product volumes.

    Returns:
        Updated cage weight and volume.
    """
    
    if quantity == 0:
        return c_weight, c_vol

    # assign the quantity of product
    trips[trip][cage].extend([product] * quantity)

    # update the weights and volumes
    c_weight += weights_dict[product] * quantity
    c_vol += vol_dict[product] * quantity

    # deduct the assigned quantity from remaining demand
    shop_dem[product] -= quantity

    # remove demand altogether if all products have been assigned
    if shop_dem[product] == 0:
        del shop_dem[product]

    return c_weight, c_vol


def open_new_cage(
    trip:int, 
    cage:int, 
    c_weight:float, 
    c_vol:float, 
    trips:dict[int,dict[int,list[int]]], 
    cages_per_trip:int
) -> tuple[int,int,float,float]:
    """Opens a new cage and updates relevant returns.

    Args:
        trip: Trip number.
        cage: Cage number.
        c_weight: Used weight capacity from already-assigned products.
        c_vol: Used volume capacity from already assigned products.
        trips: Current trips dictionary.

    Returns:
        Updated trip and cage number, and resets cage weight and volume.
    """
    
    if cage < cages_per_trip:
        cage += 1
    else:
        trip += 1
        cage = 1
        trips[trip] = {}

    trips[trip][cage] = []

    c_weight, c_vol = 0.0, 0.0

    return trip, cage, c_weight, c_vol
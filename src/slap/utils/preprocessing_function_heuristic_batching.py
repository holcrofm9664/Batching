...
def preprocessing_function_heuristic(pick_data:pd.DataFrame, solution_allocation:pd.DataFrame, num_aisles:int, num_bays:int, cage_weight_capacity:float, cage_vol_capacity:float, num_prods:int, num_stores:int, fill_percent:float=0.85, cages_per_trip:int=5, look_ahead:int=75):
    ...
    pick_data, solution_allocation = clean_dataframes(pick_data=pick_data,
                                                      solution_allocation=solution_allocation)
    
    _, prod_subset, stores_subset = sample_products_stores(num_prods=num_prods,
                                                           num_stores=num_stores,
                                                           solution_allocation=solution_allocation,
                                                           pick_data=pick_data)
    
    weights_dict, vol_dict = weights_and_volumes(solution_allocation=solution_allocation,
                                                 prod_subset=prod_subset)
    
    shop_prod_dem = demands(pick_data=pick_data,
                                  prod_subset=prod_subset,
    ...
    instance = {"shop_prod_dem":shop_prod_dem,
